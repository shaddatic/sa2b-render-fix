...
)
{
    ...
    V = XMVectorPermute(M.r[3], M.r[3], SwizzleZZYY);
    ...
    V = XMVectorPermute(M.r[3], M.r[3], SwizzleYXXX);
    ...
    P0 = XMVectorNegativeMultiplySubtract(V0, V, P0);
    P1 = XMVectorNegativeMultiplySubtract(V2, V3, P1);
    P2 = XMVectorNegativeMultiplySubtract(V4, V5, P2);

    V0 = XMVectorPermute(M.r[1], M.r[1], SwizzleWWWZ);
    V = XMVectorPermute(M.r[1], M.r[1], SwizzleZZYY);
    V2 = XMVectorPermute(M.r[1], M.r[1], SwizzleYXXX);

    S = _mm_mul_ps(M.r[0], Sign);
    R = _mm_mul_ps(V0, P0);
    R = XMVectorNegativeMultiplySubtract(V, P1, R);
    R = XMVectorMultiplyAdd(V2, P2, R);
...
//    V0 = XMVectorSelect(A, R0, g_XMSelect1110);
    V = _mm_shuffle_ps(R1,R2,_MM_SHUFFLE(2,1,2,0));
...
//    V0 = XMVectorPermute(Q1, Constant1110,Permute0Y0X0X1W);
    V = _mm_shuffle_ps(Q1,Q1,_MM_SHUFFLE(3,1,2,2));
    V = _mm_and_ps(V,g_XMMask3);
...
//    V0 = XMVectorPermute(Quaternion, Quaternion,SwizzleXXYW);
    V = _mm_shuffle_ps(Q0,Q0,_MM_SHUFFLE(3,2,1,2));
//    V1 = XMVectorPermute(Q0, Q0,SwizzleZYZW);
    V0 = _mm_mul_ps(V0, V);

    V = _mm_shuffle_ps(Quaternion,Quaternion,_MM_SHUFFLE(3,3,3,3));
//    V1 = XMVectorSplatW(Quaternion);
...
//    V0 = XMVectorPermute(R1, R2,Permute0Y1X1Y0Z);
    V = _mm_shuffle_ps(R1,R2,_MM_SHUFFLE(2,2,0,0));
    V = _mm_shuffle_ps(V,V,_MM_SHUFFLE(2,0,2,0));
...
//    M.r[1] = XMVectorPermute(R0, V0,Permute1Z0Y1W0W);
    Q1 = _mm_shuffle_ps(V,R0,_MM_SHUFFLE(3,2,1,0));
    M.r[2] = Q1;
...
{
    XMVECTOR V = XMLoadFloat4((XMFLOAT4*)&Float4x4._11);
...
{
    XMVECTOR V = XMLoadFloat4A((XMFLOAT4A*)&Float4x4._11);
