...
#elif defined(_XM_SSE_INTRINSICS_)
	XMVECTOR vTemp1 = _mm_andnot_ps(Control,V);
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT( pCR );
    XMVECTOR vTemp = _mm_cmpeq_ps(V,V2);
    UINT CR = 0;
...
#elif defined(_XM_SSE_INTRINSICS_)
	__m128i V = _mm_cmpeq_epi32( reinterpret_cast<const __m128i *>(&V)[0],reinterpret_cast<const __m128i *>(&V2)[0] );
    return reinterpret_cast<__m128 *>(&V)[0];
#else // _XM_VMX128_INTRINSICS_
#endif // _XM_VMX128_INTRINSICS_
}
...
#elif defined(_XM_SSE_INTRINSICS_)
    // Get the difference
    XMVECTOR vDelta = _mm_sub_ps(V,V2);
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT( pCR );
    XMVECTOR vTemp = _mm_cmpgt_ps(V,V2);
    UINT CR = 0;
    int iTest = _mm_movemask_ps(vTemp);
    if (iTest==0xf)
    {
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMASSERT( pCR );
    XMVECTOR vTemp = _mm_cmpge_ps(V,V2);
    UINT CR = 0;
    int iTest = _mm_movemask_ps(vTemp);
    if (iTest==0xf)
    {
...
#elif defined(_XM_SSE_INTRINSICS_)
    __m128i V = _mm_andnot_si128( reinterpret_cast<const __m128i *>(&V2)[0], reinterpret_cast<const __m128i *>(&V)[0] );
    return reinterpret_cast<__m128 *>(&V)[0];
#else // _XM_VMX128_INTRINSICS_
#endif // _XM_VMX128_INTRINSICS_
}
...
#elif defined(_XM_SSE_INTRINSICS_)
    __m128i V = _mm_or_si128( reinterpret_cast<const __m128i *>(&V)[0], reinterpret_cast<const __m128i *>(&V2)[0] );
    return reinterpret_cast<__m128 *>(&V)[0];
#else // _XM_VMX128_INTRINSICS_
#endif // _XM_VMX128_INTRINSICS_
}
...
#elif defined(_XM_SSE_INTRINSICS_)
    __m128i Result;
    Result = _mm_or_si128( reinterpret_cast<const __m128i *>(&V)[0], reinterpret_cast<const __m128i *>(&V2)[0] );
...
#elif defined(_XM_SSE_INTRINSICS_)
	__m128i V = _mm_xor_si128( reinterpret_cast<const __m128i *>(&V)[0], reinterpret_cast<const __m128i *>(&V2)[0] );
    return reinterpret_cast<__m128 *>(&V)[0];
#else // _XM_VMX128_INTRINSICS_
#endif // _XM_VMX128_INTRINSICS_
}
...
#elif defined(_XM_SSE_INTRINSICS_)
    // Adjust the angles
    XMVECTOR vResult = _mm_add_ps(V,V2);
    // Less than Pi?
...
#elif defined(_XM_SSE_INTRINSICS_)
    // Adjust the angles
    XMVECTOR vResult = _mm_sub_ps(V,V2);
    // Less than Pi?
    XMVECTOR vOffset = _mm_cmplt_ps(vResult,g_XMNegativePi);
    vOffset = _mm_and_ps(vOffset,g_XMTwoPi);
    // Add 2Pi to all entries less than -Pi
...
#elif defined(_XM_SSE_INTRINSICS_)
	XMVECTOR vResult = _mm_mul_ps( V, V2 );
...
)
{
    ...
    XMVECTOR V = _mm_load_ps1(&C1.f[1]);
    V = _mm_mul_ps(V,R);
    vConstants = _mm_load_ps1(&C1.f[0]);
    V = _mm_add_ps(V,vConstants);
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMVECTOR vResult = _mm_setr_ps(
        powf(XMVectorGetX(V),XMVectorGetX(V2)),
        powf(XMVectorGetY(V),XMVectorGetY(V2)),
        powf(XMVectorGetZ(V),XMVectorGetZ(V2)),
        powf(XMVectorGetW(V),XMVectorGetW(V2)));
    return vResult;
#else // _XM_VMX128_INTRINSICS_
#endif // _XM_VMX128_INTRINSICS_
}
...
)
{
    ...
    *pCos = Cos;
    ...
    XMASSERT(pSin);
    XMASSERT(pCos);
    XMVECTOR V, V2, V3, V4, V5, V6, V7, V8, V9, V10, V11, V12, V13;
    XMVECTOR V14, V15, V16, V17, V18, V19, V20, V21, V22, V23;
    XMVECTOR S1, S2, S3, S4, S5, S6, S7, S8, S9, S10, S11;
    XMVECTOR C1, C2, C3, C4, C5, C6, C7, C8, C9, C10, C11;
    XMVECTOR Sin, Cos;
...
)
{
    ...
    V = _mm_add_ps(V,g_XMNegativeOne);
...
)
{
    ...
    V = XMVectorMultiply(V, OneOverPi);
    V = XMVectorRound(V);

    V = XMVectorNegativeMultiplySubtract(g_XMPi, V, V);

    T0 = XMVectorSplatX(g_XMTanEstCoefficients);
    T1 = XMVectorSplatY(g_XMTanEstCoefficients);
    T2 = XMVectorSplatZ(g_XMTanEstCoefficients);

    V2T2 = XMVectorNegativeMultiplySubtract(V, V, T2);
    V2 = XMVectorMultiply(V, V);
    V1T0 = XMVectorMultiply(V, T0);
    V1T1 = XMVectorMultiply(V, T1);
...
)
{
	...
	Length = _mm_sub_ps( V, V0 );

	Result = _mm_mul_ps( Length, R );
...
)
{
    ...
    //          (-2 * t^3 + 3 * t^2) * Position1 +
    //          (t^3 - t^2) * Tangent1
    XMVECTOR T2 = _mm_mul_ps(R,R);
    XMVECTOR T3 = _mm_mul_ps(R,T2);
...
#elif defined(_XM_SSE_INTRINSICS_)
    __m128i vTemp = _mm_cmpeq_epi32(reinterpret_cast<const __m128i *>(&V)[0],reinterpret_cast<const __m128i *>(&V2)[0]);
...
)
{
    ...
    return (((V1.vector4_f32[0] < V2.vector4_f32[0]) && (V1.vector4_f32[1] < V2.vector4_f32[1])) != 0);
#elif defined(_XM_SSE_INTRINSICS_)
    XMVECTOR vTemp = _mm_cmplt_ps(V,V2);
    return (((_mm_movemask_ps(vTemp)&3)==3) != 0);
#else // _XM_VMX128_INTRINSICS_
...
)
{
    ...
    return (((V1.vector4_f32[0] <= V2.vector4_f32[0]) && (V1.vector4_f32[1] <= V2.vector4_f32[1])) != 0);
#elif defined(_XM_SSE_INTRINSICS_)
    XMVECTOR vTemp = _mm_cmple_ps(V,V2);
    return (((_mm_movemask_ps(vTemp)&3)==3) != 0);
#else // _XM_VMX128_INTRINSICS_
...
)
{
    ...
    XMVECTOR vLengthSq = _mm_mul_ps(V,V2);
...
)
{
    ...
    XMVECTOR CosAngle;
    XMVECTOR Result;
    L1 = XMVector2ReciprocalLength(V);
    L2 = XMVector2ReciprocalLength(V2);
    Dot = XMVector2Dot(V, V2);
    L1 = _mm_mul_ps(L1, L2);
...
#elif defined(_XM_SSE_INTRINSICS_)
    // y1,z1,x1,w1
    XMVECTOR vTemp1 = _mm_shuffle_ps(V,V,_MM_SHUFFLE(3,0,2,1));
    // z2,x2,y2,w2
...
)
{
    ...
    L1 = XMVector3ReciprocalLength(V);
    L2 = XMVector3ReciprocalLength(V2);
    Dot = XMVector3Dot(V, V2);
    L1 = _mm_mul_ps(L1, L2);
    CosAngle = _mm_mul_ps(Dot, L1);
    CosAngle = XMVectorClamp(CosAngle,g_XMNegativeOne,g_XMOne);
    Result = XMVectorACos(CosAngle);
...
)
{
    ...
    return (((V1.vector4_f32[0] != V2.vector4_f32[0]) || (V1.vector4_f32[1] != V2.vector4_f32[1]) || (V1.vector4_f32[2] != V2.vector4_f32[2]) || (V1.vector4_f32[3] != V2.vector4_f32[3])) != 0);
#elif defined(_XM_SSE_INTRINSICS_)
    XMVECTOR vTemp = _mm_cmpneq_ps(V,V2);
...
#elif defined(_XM_SSE_INTRINSICS_)
    XMVECTOR vTemp2 = V2;
    XMVECTOR vTemp = _mm_mul_ps(V,vTemp2);
...
)
{
    ...
    // term1 * V1yxxx
    XMVECTOR vTemp1 = _mm_shuffle_ps(V,V,_MM_SHUFFLE(0,0,0,1));
    ...
    // vResult - temp * V1zzyy
    vTemp1 = _mm_shuffle_ps(V,V,_MM_SHUFFLE(1,1,2,2));
    ...
    // vResult + term * V1wwwz
    vTemp1 = _mm_shuffle_ps(V,V,_MM_SHUFFLE(2,3,3,3));
...
)
{
    ...
    L1 = XMVector4ReciprocalLength(V);
    L2 = XMVector4ReciprocalLength(V2);
    Dot = XMVector4Dot(V, V2);
    L1 = _mm_mul_ps(L1,L2);
    CosAngle = _mm_mul_ps(Dot,L1);
    CosAngle = XMVectorClamp(CosAngle, g_XMNegativeOne, g_XMOne);
    Result = XMVectorACos(CosAngle);
