...
)
{
    ...
    XMVECTOR R = _mm_shuffle_ps( V, V, _MM_SHUFFLE( 1, 1, 1, 1 ) );
    _mm_store_ss( (float*)&pDestination[0], V );
    _mm_store_ss( (float*)&pDestination[1], R );
#else // _XM_VMX128_INTRINSICS_
#endif // _XM_VMX128_INTRINSICS_
}
...
)
{
    ...
    XMVECTOR R = _mm_shuffle_ps(V,V,_MM_SHUFFLE(2,2,2,2));
    _mm_storel_epi64( (__m128i*)pDestination, reinterpret_cast<const __m128i *>(&V)[0] );
    _mm_store_ss( (float*)&pDestination[2], R );
...
)
{
    ...
    for(j=0; j < 3; ++j)
    {
        ...
        else
        {
            ...
            R = ((I + 0x1FFFU + ((I >> 14U) & 1U)) >> 14U)&0x3fffU;
    ...
    R = XMMax( Exp[0], XMMax( Exp[1], Exp[2] ) );

    Frac[0] = Frac[0] >> (R - Exp[0]);
    Frac[1] = Frac[1] >> (R - Exp[1]);
    Frac[2] = Frac[2] >> (R - Exp[2]);
