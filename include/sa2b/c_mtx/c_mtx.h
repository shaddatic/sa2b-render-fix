...
void    C_MTXMultVec(const GJS_MATRIX* m, const NJS_VECTOR* vs, NJS_VECTOR* vd, bool sr);

/** Multiply vector with only scale and rotation (different function from above) **/
void    C_MTXMultVecSR(const GJS_MATRIX* m, const NJS_VECTOR* vs, NJS_VECTOR* vd);
